...
## 双语文档维护（README 与 docs 六件套均为中英双语）

- 结构：`README.md` / `docs/*.md` 为英文（默认），`README.zh-CN.md` / `docs/zh-CN/*.md` 为中文；每份文件标题下有语言切换行互指同主题文件
- **改英文必须同步中文**：任何对英文版的实质内容修改，必须在同一提交里同步对应中文版；中文版 README 尾部「同步基线」标记随同步更新，来不及同步时保留旧标记暴露滞后，禁止静默分叉
- 新增公开文档必须同时产出英文与中文两份，并在两份标题下加互指切换行
- 术语对译以 `Docs/01_需求与规划/README与docs多语言支持执行计划.md` 附录术语表为准（composite=组合控制流、checkpoint=人工确认点等 15 条），不得各翻各的
- 代码示例块两边保持一致（代码不翻译，注释可按所属语言版本调整）
