...
|------|--------|------|------|
| 3 | TA-05 双插件配置 | 通过 | |
| 3 | TA-06 不 fork | 通过 | 无 OpenCode 源码改动 |
| 3 | TA-07 v1 only | 通过 | |
| 4 | TA-01a 前台实时树 | 通过 | 图标/分组/计数/实时性 |
| 5 | TA-02 完成态保持 | 通过 | 含折叠 |
| 6 | 中止态快照 | 通过 | |
| 7 | 失败态展示 | 通过 | ✖ 与错误行 |
| 8 | TA-01b 后台实时树 | 通过 | v0.3 起正向 |
| 9 | 空会话不渲染 | 通过 | |
| 12 | MVP-2 sidebar 点击进子会话 | 通过 | |
| 13 | MVP-3 workflow 全屏路由 | 通过 | 键盘导航 |
| 14 | 输入框右侧运行中状态条 | 通过 | | 仅 running 态显示 |
| 15 | 全屏路由选中行滚动跟随 | 通过 | | 超屏树时 j/k 越屏自动滚动 |

---

## 14. 用例 14 输入框右侧 workflow 状态条

步骤：跑一轮 workflow（前台或后台均可），运行期间看输入框右侧。

判定：运行期间显示一行黄色 `◐ workflow 名字 完成 或 总数 · N running`；完成后消失（终态由 sidebar 与聊天区负责）。无 workflow 运行时不显示。

## 15. 用例 15 全屏路由选中行滚动跟随

步骤：跑一个多 agent 的 workflow（可临时把脚本里 parallel 数组复制成十几个 agent），全屏路由里连续按 j 让 ▸ 越出屏幕。

判定：选中行接近屏幕边缘时视图自动滚动跟随，▸ 始终可见。
